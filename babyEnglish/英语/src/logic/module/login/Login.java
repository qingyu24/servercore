...
public class Login implements LoginInterface, Tick {
	 ...
	 */
	@Override
	public void Enter(MyUser p_user, String p_username, String p_password, int userType, int p_nServerID,
			String p_deviceIdentifier, String p_deviceModel) {
		InternalLogin(p_user, p_username, p_password, userType, p_nServerID, p_deviceIdentifier, p_deviceModel, 0,
				false);
	}

	private boolean Validate(MyUser p_user, String p_username, String p_password, int p_nServerID) {
...
/*		if(!CheckLoginRecords(m_All.get(p_user.GetServerUserName()), p_user, p_username, p_password)){
			return false;
		}*/
	...
	public static void  Check(MyUser p_user,String p_username, String p_password){
	 ...
	 * 主要登录逻辑是在LoginMethod和LoginSQLRun，这里只是做有效性检测以及登录队列的处理
	 */
	private void InternalLogin(MyUser p_user, String p_username, String p_password, int userType, int p_nServerID,
			String deviceIdentifier, String deviceModel, int code, boolean isRegister) {
		Log.out.Log(eLogicInfoLogType.LOGIC_COMMON,
				"Login#Enter:" + p_username + "," + p_nServerID + "," + Debug.GetCurrTime());
		// 检查服务器是否已经满员
		...
		if (RootConfig.GetInstance().UsePasswordVerify) {
			m_WaitSQLRuns.put(p_username, new LoginMethod(p_user, new LoginSQLRun(p_user, p_username, p_password,
					userType, p_nServerID, deviceIdentifier, deviceModel, code, isRegister)));
		}

		boolean c = m_LoginWait.add(p_username);
		Log.out.Log(eLogicInfoLogType.LOGIC_COMMON, "Login#Enter#AddLoginWait:" + p_username + "," + p_nServerID + ","
				+ Debug.GetCurrTime() + ",result:" + c + ",size:" + m_LoginWait.size());
		// 通知客户端前头还有多少人在等待登录
		PackBuffer.GetInstance().Clear().AddID(Reg.LOGIN, 10).Add(m_LoginWait.size()).Send(p_user);
	}

	private void InternalRegister(MyUser p_user, String p_username, String p_password, int userType, String tel,
			int code, String pass, int isMember, int groupID, String groupName, String Area, String Province,
			String city, String mail) {
		Log.out.Log(eLogicInfoLogType.LOGIC_COMMON,
				"Login#Register:" + p_username + "," + 0 + "," + Debug.GetCurrTime());
		// 检查服务器是否已经满员
		...
		m_WaitSQLRuns.put(p_username, new LoginMethod(p_user, new LoginSQLRun(p_user, p_username, p_password, userType,
				tel, code, pass, isMember, groupID, groupName, Area, Province, city, mail)));

		boolean c = m_LoginWait.add(p_username);
		Log.out.Log(eLogicInfoLogType.LOGIC_COMMON, "Login#Enter#AddLoginWait:" + p_username + "," + 0 + ","
				+ Debug.GetCurrTime() + ",result:" + c + ",size:" + m_LoginWait.size());
		// 通知客户端前头还有多少人在等待登录
		PackBuffer.GetInstance().Clear().AddID(Reg.LOGIN, 10).Add(m_LoginWait.size()).Send(p_user);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see logic.module.login.LoginInterface#Binary(logic.MyUser,
	 * java.lang.String)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see logic.module.login.LoginInterface#CreateWebRole(logic.MyUser,
	 * java.lang.String, java.lang.String, int, int, java.lang.String, int)
	 */
	@Override
	public void CreateWebRole(MyUser p_user, String p_username, String p_password, int p_nAdult, int p_nServerID,
			String p_RoleName, int p_TemplateID) {
		// todo by niuhao;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see logic.module.login.LoginInterface#Echo(logic.MyUser, int,
	 * java.lang.String)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see logic.module.login.LoginInterface#EchoReceive(logic.MyUser, int,
	 * java.lang.String)
	 */
	@Override
	...
	public void EchoSend(MyUser p_user, int p_nHashcode, int p_nLength) {
		PackBuffer.GetInstance().Clear().AddID(Reg.LOGIN, MID_ECHOSEND).Add(p_nHashcode)
				.Add(new String(m_EchoSendBytes, 0, p_nLength)).Send(p_user);
	}

	/*
	 * (non-Javadoc)
	 * 
	 * @see logic.module.login.LoginInterface#LoadByUserName(logic.MyUser,
	 * logic.MyUser)
	 */
	@Override
	...
	public void OnDisconnect(MyUser p_User) {
		...
		if (m_All.containsKey(username)) {
			...
			if (r.GetHashCode() == p_User.hashCode()) {
				// System.out.println("* Login#OnDisconnect 删除:" +
				// p_User.GetUserName() + "登陆数据");
	...
	public void OnTick(long p_lTimerID) throws Exception {
		...
		while (m_Logining.size() < scc.LoginMaxWaitNum && max > 0) {
			...
			Log.out.Log(eLogicInfoLogType.LOGIC_COMMON,
					"Login#OnTick#LoginWaitPoll:" + username + "," + Debug.GetCurrTime() + ",result:");
			// 依照用户名得到登录实例
			...
			// Log.out.Log(eLogicInfoLogType.LOGIC_COMMON, "*
			// Login#EnterLoginInfo:" + m.GetUser() + ", loginingSize:" +
			// m_Logining.size() + ", loginWaitSize:" + m_LoginWait.size());
		}
	}
	...
	public void SetLoginCreateRole(MyUser p_user) {
		...
		if (r.GetHashCode() != p_user.hashCode()) {
			System.out.println("* SetLoginCreateRole 有:" + p_user.GetUserName() + "的登陆数据,但是不是自己:" + p_user.hashCode()
					+ ",而是" + r.GetHashCode());
	...
	public void SetLoginCreateRoleFinish(MyUser p_user) {
		...
		if (r.GetHashCode() != p_user.hashCode()) {
			System.out.println("* SetLoginCreateRoleFinish 有:" + p_user.GetUserName() + "的登陆数据,但是不是自己:"
					+ p_user.hashCode() + ",而是" + r.GetHashCode());
	...
	public void SetLoginFail(MyUser p_user) {
		...
		if (r == null) {
			...
			if (r.GetHashCode() != p_user.hashCode()) {
				System.out.println("* SetLoginFail 有:" + p_user.GetUserName() + "的登陆数据,但是不是自己:" + p_user.hashCode()
						+ ",而是" + r.GetHashCode());
		...
		// p_user.Close(eLogicCloseUserReason.UPDATE_SET_LOGINDATA_FAIL.ID(),
		// 0);
	}
	...
	public boolean SetLoginSuccess(MyUser p_user) {
		...
		if (r.GetHashCode() != p_user.hashCode()) {
			System.out.println("* SetLoginSuccess 有:" + p_user.GetUserName() + "的登陆数据,但是不是自己:" + p_user.hashCode()
					+ ",而是" + r.GetHashCode());
			eLoginDebugLogType.LOGIN_OK_ERR_CHANGE_RECORD.Log(p_user);
			return false;
		}
		if (r.GetLoginUseTime() > m_WaitLoginTime) {
			System.out.println(" SetLoginSuccess 虽然登陆成功了,但是超时了,现在是:" + Debug.GetShowTime(System.currentTimeMillis())
					+ ",登陆是:" + Debug.GetShowTime(r.GetLoginTime()));
	...
	public void SQLLog(MyUser p_user) {
		p_user.Log(eLogicSQLLogType.LOGIC_SQL_LOGIN, RootConfig.GetInstance().ServerUniqueID,
				eLogicSQLLogType.GetCurrTime(), 0, "", p_user.GetLink().GetIP(), "", "", 9527);
	}
	...
	private static boolean _KillOpposite(MyUser p_User) {
	...
	private void _SToC_LoginReturn(MyUser p_user, int errid, String username) {
		...
		p_user.Log(eLogicSQLLogType.LOGIC_SQL_LOGIN, RootConfig.GetInstance().ServerUniqueID,
				eLogicSQLLogType.GetCurrTime(), 0, username, p_user.GetLink().GetIP(), "", "", errid);
	...
	private static boolean CheckLoginRecords(LoginRecord r, MyUser p_user, String p_username, String p_password) {
		if (r.GetState() == LoginState.LOGINING || r.GetState() == LoginState.LOGIN_CREATE
			...
			if (r.GetLoginUseTime() > m_WaitLoginTime) {
				...
				// System.out.println("* 前面的用户已经登陆超过:"+m_WaitLoginTime+"毫秒,用户" +
				// r.GetHashCode() + ",登陆用时间为," + r.GetLoginUseTime() +
				// ",不应该出现这种情况,如果出现了应该是系统出问题了,也先忽略这个用户登陆,高级别警告");
	...
	private static boolean CheckLoginRecord(LoginRecord r, MyUser p_user, String p_username, String p_password) {
		if (r.GetState() == LoginState.LOGINING || r.GetState() == LoginState.LOGIN_CREATE
				|| r.GetState() == LoginState.LOGIN_CREATEFINISH) {
			if (r.GetLoginUseTime() > m_WaitLoginTime) {
				eLoginDebugLogType.REPLACE.Log(p_user);
			/*	System.out.println("* 时间太久导致的断线？？？？？");*/
				// TODO
				// 实际情况是在某些情况下客户端断开,服务器并不清楚.需要在连接那添加一个不断用来ping的包来测试断开问题.这样就导致这个号重新上的时候还能发现登陆数据
				return _KillOpposite((MyUser) Root.GetInstance().GetUserByUserName(p_username));
				// eLoginDebugLogType.ERR_TOOLONG.Log(p_user);
				// System.out.println("* 前面的用户已经登陆超过:"+m_WaitLoginTime+"毫秒,用户" +
				// r.GetHashCode() + ",登陆用时间为," + r.GetLoginUseTime() +
...
/*				if (r.GetHashCode() == p_user.hashCode()) {
		...
		/*			if (loginRight) {*/
						System.out.println("密码正确即将被顶掉");
						return _KillOpposite((MyUser) Root.GetInstance().GetUserByUserName(p_username));
					
				/*	}*/

					/*
					 * System.out.println(" 另外一个用户:" + p_user.hashCode() + " 想挤掉用户:"
					 * + r.GetHashCode() + ",正在执行!!!"); return
					 * _KillOpposite((MyUser)
					 * Root.GetInstance().GetUserByUserName(p_username));
					 */
				}
	...
	public void Register(@PU MyUser p_user, @PS String p_username, @PS String p_password, @PI int userType,
			@PS String tel, @PI int code, @PS String pass, @PI int isMember, @PI int groupID, @PS String groupName,
			@PS String area, @PS String Province, @PS String city, @PS String mail) {
		// TODO Auto-generated method stub
		InternalRegister(p_user, p_username, p_password, userType, tel, code, pass, isMember, groupID, groupName, area,
				Province, city, mail);
	}

	@Override
	@RFC(ID = 3, RunDirect = true)
	public void ResetEnter(@PU MyUser p_user, @PS String p_username, @PS String p_password, @PI int userType,
			@PI int p_nServerID, @PS String p_deviceIdentifier, @PS String p_deviceModel, @PI int code) {
		// TODO Auto-generated method stub
		InternalLogin(p_user, p_username, p_password, userType, p_nServerID, p_deviceIdentifier, p_deviceModel, code,
				false);
	}
