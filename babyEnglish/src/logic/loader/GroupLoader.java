...
public class GroupLoader extends DBLoaderEx<bs_group> {
	...
	public void updateUserCount(int gid, int count) {
		...
		/*	select count(*)from bs_user where groupid=666666;*/
/*			update bs_group set number=(select count(*)from bs_user where groupid=666666) where gid=666666;*/

			boolean sql = DBMgr.ExecuteSQL("update bs_group set number=(select count(*)from bs_user where groupid="+gid+") where gid="+gid);
			if(sql){
				LogRecords.Log(null, "用户数量更新");}
		}
	}
	...
	public  void reset() {
		...
		i+=1000;
			bs_group bs_group = (bs_group) iterator.next();
			bs_group.RoleID.Set(i);
			DBMgr.ExecuteSQL("update bs_group  set roleid="+i+" where gid="+bs_group.GID.Get());
